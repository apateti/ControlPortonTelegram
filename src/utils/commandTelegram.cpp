...
String comandSendLocation = "/sendLocation";
//
    ...
    if (updateID == 0){
      ...
      if(root["result"][0]["callback_query"]["id"]){
        ...
        if (root["result"][0]["message"]["text"]) {
          ...
          userMsg.location.latitude = root["result"][0]["message"]["location"]["latitude"];
          userMsg.location.longitude = root["result"][0]["message"]["location"]["longitude"];
          userMsg.messageType = MessageLocation;
        }
...
int sendComand(int32_t userId, String mensaje, String formato){
  ...
  #if DEBUG_MODE > 0
  Serial.println("Rutina sendComand:");
  Serial.println(json);
...
int sendUbicacion(int32_t userId, float latit, float longit){
  ...
  http.end();
  #if DEBUG_MODE > 0
    Serial.print("\nRespuesta Ubicacion: "); Serial.println(httpResponseCode);
  #endif
  return (httpResponseCode);
}
...
int sendMenu(int32_t userId){
  ...
  json = "{\"keyboard\":[[\"Abrir_Porton\",\"Cerrar_Porton\"],[\"Estatus_Porton\"],[\"Temperatura\"],[\"Power_ON_OFF\"],[\"Ubicacion\"]],\"resize_keyboard\":true}";
  StaticJsonDocument<1000> root;
  root.clear();
  root["chat_id"] = userId;
